...
class UpdatePendingRepairHistory(models.Model):
    ...
    pending_repair_ids = fields.Many2many("pending.repair.type",
    ...
    state = fields.Selection([('draft', 'New'), ('confirm', 'Applied')], string="Status", default='draft')
    note = fields.Text(string="Note", translate=True)
    pending_repair_history_ids = fields.One2many('pending.applied.history', "updated_pending_id", string="Pending Applied History", readonly=True)
    ...
    def remove_selected_pending(self):
        ...
        for pending in self:
            for applied_pending in pending.pending_repair_ids:
                applied_pending_obj.create({
                    'updated_pending_id': pending.id,
                    'vehicle_id': applied_pending.vehicle_id and applied_pending.vehicle_id.id or False,
                    'repair_type_id': applied_pending.repair_type_id and applied_pending.repair_type_id.id or False,
                    'name': applied_pending.name or "",
                    'categ_id': applied_pending.categ_id and applied_pending.categ_id.id or False,
                    'issue_date': applied_pending.issue_date,
                    'state': "complete",
                    "user_id": applied_pending.user_id and applied_pending.user_id.id or False})
                applied_pending.unlink()
            pending.write({'state': 'confirm', 'user_id': self._uid})
        return True

UpdatePendingRepairHistory()
...
class PendingAppliedHistory(models.Model):
    ...
    updated_pending_id = fields.Many2one("update.pending.repair.history", string="Update Pending")
    ...
    state = fields.Selection([('complete', 'Complete'), ('in-complete', 'In Complete')], string="Status")
    user_id = fields.Many2one('res.users', string="By")

PendingAppliedHistory()